...
class Project(db.Model):
    ...
    status = db.Column(db.Enum('Pre-Sales', 'Ongoing', 'Maintenance', 'Completed', name='project_status'), default='Ongoing')
...
class PersonalSchedule(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    report_id = db.Column(db.Integer, db.ForeignKey('report.id'), nullable=False)
    category = db.Column(db.Enum('출장', '외근', '휴가', '휴일근무', name='schedule_category'), nullable=False)
    person = db.Column(db.String(100), nullable=False)
    location = db.Column(db.String(100), nullable=False)
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
    start_time = db.Column(db.Time)  # optional
    end_time = db.Column(db.Time)    # optional
