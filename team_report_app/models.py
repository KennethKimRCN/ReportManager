...
class Update(db.Model):
    ...
    week = db.Column(db.String(10))  # e.g. Y25W30
...
class SalesSupport(db.Model):
    ...
    system = db.Column(db.String(100))     # 시스템 (e.g., MPA)
...
class PersonalSchedule(db.Model):
    ...
    person = db.Column(db.String(100))       # 예: 홍길동 사원
    location = db.Column(db.String(100))     # 예: 여수
    start_date = db.Column(db.String(20))    # YYYY-MM-DD
    end_date = db.Column(db.String(20))      # YYYY-MM-DD
    description = db.Column(db.Text)         # 비고 내용
